...
// -- START CORRECT ROUTING ORDER --

// 1. All API endpoints with SSO middleware should be defined first.
...
let pgPool = null;
const enablePg = !!process.env.PGHOST;
if (enablePg) {
    pgPool = new Pool({
...
// Health check endpoint without SSO
...
app.post('/api/expand-image', upload.single('image'), nsfwGuard('prompt'), async (req, res) => {
    const imagePath = req.file.path;
    try {
      const { prompt } = req.body;
      if (!prompt || !imagePath) {
      ...
      await logPrompt(req, '/api/expand-image', prompt);
      const imageBuffer = fs.readFileSync(imagePath);
      const imageBase64 = imageBuffer.toString('base64');
      const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash-image-preview" });
      const requestBody = {
      ...
      const result = await model.generateContent(requestBody);
      const response = await result.response;
      console.log('Full AI Expand Response:', JSON.stringify(response, null, 2));
      if (response && response.candidates && response.candidates[0] && response.candidates[0].content && response.candidates[0].content.parts) {
          const imagePart = response.candidates[0].content.parts.find(part => part.inlineData);
          if (imagePart) {
              const imageUrl = `data:${imagePart.inlineData.mimeType};base64,${imagePart.inlineData.data}`;
          ...
          return res.status(500).json({ message: 'Could not parse expanded image from AI response.' });
      }
      return res.status(500).json({ message: 'Unexpected AI response structure for expanding image.' });
    } catch (error) {
      console.error('Error expanding image:', error);
      return res.status(500).json({ message: 'Error expanding image', error: error.message });
    } finally {
      try { fs.unlinkSync(imagePath); } catch (_) {}
    }
});

app.post('/api/combine-images', upload.array('images'), nsfwGuard('prompt'), async (req, res) => {
    const uploadedFiles = req.files || [];
    try {
      const { prompt } = req.body;
      if (!prompt || uploadedFiles.length < 2) {
        return res.status(400).json({ message: 'Prompt and at least two images are required.' });
      }
      await logPrompt(req, '/api/combine-images', prompt);
      
      // Read all uploaded images and convert to base64
...
console.log('Does this path exist?', fs.existsSync(distPath));

// 2. Serve the static files from your React build.
app.use(express.static(path.join(__dirname, '../client/dist')));

// 3. Catch-all route to serve index.html for any other request.
